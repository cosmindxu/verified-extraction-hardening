...
/* Safe Rust API over the extracted world-frame scene checker. */
...
fn objs_from<'a>(p: &'a Program, os: &[SceneObj]) -> &'a WObjList<'a> {
    let mut acc: &'a WObjList<'a> = p.alloc(Corelib_Init_Datatypes_list::nil(PhantomData));
...
/// World-frame check: ego pose (px, py, heading quarter-turns, world
/// velocity) + world-frame objects. Ingestion (translate + rotate) runs
/// INSIDE the verified code; outside the proved SAFE_COORD domain the
/// checked arithmetic panics (contained by the FFI layer).
pub fn check_world(
    ...
    let rep = p.RocqRustExamples_SceneWorld_scene_world_demo(
        rd, px, py, ph, pvx, pvy, objs_from(&p, objs),
    );
    match rep {
